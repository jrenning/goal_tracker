...
const levels = level_data ? level_data.map(item => {return item.level}) : []
const level_dates = level_data ? level_data.map(item=> {return item.date.toDateString()}) : []
const points = point_data ? point_data.map(item=> {return item.points}) : []
const point_dates = point_data ? point_data.map(item=> {return item.date.toDateString()}) : []
...
const data: Dataset[] = [
  ...
  return (
    <div className='flex justify-center items-center mt-4'>
        <LineChart labels={type == "Points" ? point_dates : level_dates} datasets={data} />
    </div>
  )
}
